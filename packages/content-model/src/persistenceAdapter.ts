...
export function validatePersistenceAdapterPlan(plan: PersistenceAdapterPlan): string[] {
  ...
  for (const intent of plan.writeIntents) {
    ...
    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.preservesAiPrototypeAppPatchProposal) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must preserve proposal sections.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.requiresCodexIntegrationReviewDecision) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must require Codex integration decisions.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.requiresAllPrototypeEvidenceReviewed) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must require all prototype evidence reviewed.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.requiresProposedPatchFileScope) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must require proposed patch file scope.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.requiresPrePatchGates) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must require pre-patch gates.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.requiresPatchTestGates) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must require patch test gates.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.requiresReviewerIdentitySignatureGate) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must require reviewer identity signature gates.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.requiresReleaseControlBinding) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must require release-control binding.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.blocksAppFileWrite) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must block app file writes.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.blocksAppPatchGeneration) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must block app patch generation.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.blocksDirectAppImport) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must block direct app imports.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.blocksGeneratedGameRouteWrite) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must block route writes.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.blocksStudentFacingRoute) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must block student-facing routes.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.blocksScoringProfileOverride) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must block scoring mutations.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.blocksStarDustWrite) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must block Star Dust writes.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.blocksRewardInventoryWrite) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must block reward inventory writes.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.blocksAudioManifestMutation) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must block audio manifest mutations.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.blocksPackagePromotion) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must block package promotion.`);
    }

    if (intent.category === "ai-prototype-app-patch-proposal" && !intent.blocksDirectStudentAssignment) {
      errors.push(`AI prototype app patch proposal write intent ${intent.intentId} must block student assignments.`);
    }
    ...
    if (
      intent.category === "ai-prototype-patch-test-harness-plan" &&
      !intent.preservesAiPrototypePatchTestHarnessPlan
    ) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must preserve harness plan sections.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.requiresHarnessRuntimePolicy) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must require harness runtime policy.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.requiresHarnessRequiredInputs) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must require harness inputs.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.requiresHarnessSectionCoverage) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must require harness section coverage.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.requiresHarnessNonExecutionOutputs) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must require non-execution outputs.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.blocksTestExecution) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must block test execution.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.blocksPlaywrightRun) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must block Playwright runs.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.blocksAppFileWrite) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must block app file writes.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.blocksAppPatchGeneration) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must block app patch generation.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.blocksGeneratedGameRouteWrite) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must block route writes.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.blocksStudentFacingRoute) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must block student-facing routes.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.blocksScoringProfileOverride) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must block scoring mutations.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.blocksStarDustWrite) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must block Star Dust writes.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.blocksRewardInventoryWrite) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must block reward inventory writes.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.blocksAudioManifestMutation) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must block audio manifest mutations.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.blocksPackagePromotion) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must block package promotion.`);
    }

    if (intent.category === "ai-prototype-patch-test-harness-plan" && !intent.blocksDirectStudentAssignment) {
      errors.push(`AI prototype patch test harness plan write intent ${intent.intentId} must block student assignments.`);
    }
    ...
    if (
      intent.category === "ai-generated-package-writer-implementation-readiness" &&
      !intent.preservesAiGeneratedPackageWriterImplementationReadiness
    ) {
      errors.push(`AI generated package writer implementation readiness write intent ${intent.intentId} must preserve implementation readiness gates.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer implementation readiness write intent ${intent.intentId} must preserve rollback drill links.`);
    }

    if (
      intent.category === "ai-generated-package-writer-implementation-readiness" &&
      !intent.requiresPackageWriterModulePlan
    ) {
      errors.push(`AI generated package writer implementation readiness write intent ${intent.intentId} must require package writer module plans.`);
    }

    if (
      intent.category === "ai-generated-package-writer-implementation-readiness" &&
      !intent.requiresPackageWriterTestGates
    ) {
      errors.push(`AI generated package writer implementation readiness write intent ${intent.intentId} must require package writer test gates.`);
    }

    if (
      intent.category === "ai-generated-package-writer-implementation-readiness" &&
      !intent.requiresPackageWriterReleaseControls
    ) {
      errors.push(`AI generated package writer implementation readiness write intent ${intent.intentId} must require package writer release controls.`);
    }

    if (
      intent.category === "ai-generated-package-writer-implementation-readiness" &&
      !intent.requiresCodexPackageWriterImplementationDecision
    ) {
      errors.push(`AI generated package writer implementation readiness write intent ${intent.intentId} must require a Codex package writer implementation decision.`);
    }

    if (
      intent.category === "ai-generated-package-writer-implementation-readiness" &&
      !intent.blocksGeneratedPackageWriterImplementation
    ) {
      errors.push(`AI generated package writer implementation readiness write intent ${intent.intentId} must block package writer implementation.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer implementation readiness write intent ${intent.intentId} must block package writer execution.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer implementation readiness write intent ${intent.intentId} must block generated package JSON writes.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer implementation readiness write intent ${intent.intentId} must block route registry writes.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer implementation readiness write intent ${intent.intentId} must block media playlist writes.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer implementation readiness write intent ${intent.intentId} must block local bundle writes.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer implementation readiness write intent ${intent.intentId} must block assignment writes.`);
    }

    if (
      intent.category === "ai-generated-package-writer-implementation-readiness" &&
      !intent.blocksStudentReadyMarker
    ) {
      errors.push(`AI generated package writer implementation readiness write intent ${intent.intentId} must block student-ready markers.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer implementation readiness write intent ${intent.intentId} must block support-language-only implementation evidence.`);
    }

    if (
      intent.category === "ai-generated-package-writer-module-test-plan" &&
      !intent.preservesAiGeneratedPackageWriterModuleTestPlan
    ) {
      errors.push(`AI generated package writer module test plan write intent ${intent.intentId} must preserve module test plans.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer module test plan write intent ${intent.intentId} must preserve implementation readiness links.`);
    }

    if (
      intent.category === "ai-generated-package-writer-module-test-plan" &&
      !intent.requiresPackageWriterModuleTestSuites
    ) {
      errors.push(`AI generated package writer module test plan write intent ${intent.intentId} must require module test suites.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer module test plan write intent ${intent.intentId} must require writer test evidence.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer module test plan write intent ${intent.intentId} must block package writer test execution.`);
    }

    if (intent.category === "ai-generated-package-writer-module-test-plan" && !intent.blocksPlaywrightRun) {
      errors.push(`AI generated package writer module test plan write intent ${intent.intentId} must block writer mutation browser runs.`);
    }

    if (intent.category === "ai-generated-package-writer-module-test-plan" && !intent.blocksAppFileWrite) {
      errors.push(`AI generated package writer module test plan write intent ${intent.intentId} must block app file patches.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer module test plan write intent ${intent.intentId} must block generated package JSON writes.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer module test plan write intent ${intent.intentId} must block route registry writes.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer module test plan write intent ${intent.intentId} must block media playlist writes.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer module test plan write intent ${intent.intentId} must block local bundle writes.`);
    }
    ...
    if (
      ...
      errors.push(`AI generated package writer module test plan write intent ${intent.intentId} must block assignment writes.`);
    }
...
function validateAiGeneratedPackageWriterTestHarnessPlanIntent(
  ...
  if (intent.category !== "ai-generated-package-writer-test-harness-plan") {
    return;
  }

  const prefix = `AI generated package writer test harness plan write intent ${intent.intentId}`;

  if (!intent.preservesAiGeneratedPackageWriterTestHarnessPlan) {
    errors.push(`${prefix} must preserve test harness plans.`);
  }

  if (!intent.preservesAiGeneratedPackageWriterTestEvidencePacket) {
    errors.push(`${prefix} must preserve test evidence packet links.`);
  }

  if (!intent.requiresPackageWriterHarnessPhases) {
    errors.push(`${prefix} must require harness phases.`);
  }

  if (!intent.requiresPackageWriterHarnessAdapters) {
