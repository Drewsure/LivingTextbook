...
try {
  ...
  const binding = {
    bindingId: "binding-1", manifestId: "manifest-1", tenantId: "tenant-1", bundleId: "bundle-1", packageId: "package-1", packageVersion: "2026.1", unitIds: ["unit-1"], storageSelectionPreflightId: "storage-preflight-1", storageSelectionGateId: "storage-gate-1", storageSelectionStatus: "blocked", storageSelectionAllowed: false, mode: "review-only", status: "blocked",
    assets: [
      ...
      { assetId: "image", unitId: "unit-1", label: "Image", kind: "image", relativePath: "media/image.png", sourceRef: "source", rightsStatus: "unknown", rightsEvidenceRef: "rights", checksum: "missing", scanStatus: "pending", targetMappingReviewed: false, transcriptOrCaptionRef: null, posterRef: null, altTextReady: false, localEligibility: "blocked", blockers: ["review"] },
    ],
  ...
  assert(validateLocalBundleMediaEvidenceBinding({ ...binding, unitIds: [] }).some((error) => error.includes("unit scope")), "media evidence must declare unit scope");
  assert(validateLocalBundleMediaEvidenceBinding({ ...binding, assets: binding.assets.map((asset) => asset.assetId === "image" ? { ...asset, unitId: "unit-2" } : asset) }).some((error) => error.includes("outside media evidence package unit scope")), "media evidence must reject an asset outside unit scope");
