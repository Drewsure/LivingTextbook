...
for (const [label, source] of [["student session time bounds", studentSession], ["teacher session time bounds", teacherSession]]) {
  requireFragments(label, source, ["COOKIE_MAX_BYTES", "Buffer.byteLength(cookieValue, \"utf8\")", "segments.length !== 2", "hasBoundedString", "issuedAt > now + 30_000", "expiresAt <= issuedAt"]);
}
