...
The current Living Textbook build proves student progression, route resolution, content package review concepts, multimedia events, Training Academy recovery, Speak It practice, and teacher-visible summaries using static sample data and local state. A real white-label pilot needs durable records, but choosing a backend too early can increase cost, lock in the wrong deployment shape, and complicate closed/local partner deployments.
