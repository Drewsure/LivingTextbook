...
3. Keep the heavy shell routes fully checked sequentially before the concurrent sweep and exclude them from the worker pool: `/`, `/teacher`, `/teacher/intake`, `/teacher/persistence`, `/teacher/session-settings`, `/teacher/reporting`, `/teacher/game-readiness`, `/teacher/generator/sample-publisher`, and `/teacher/generator/ministar`.
