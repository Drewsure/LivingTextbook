...
- The route path, content package, access policy, launch session, progression,
  and permanent QR path all remain tenant-bound.
...
- The review-only adapter always reports no side effect and cannot mutate QR
...
- `printed_qr_id`, current and previous release identity, target/fallback,
