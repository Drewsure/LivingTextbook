...
- Confirm every active playable mode has a route contract and helper, including `match-up` at `/match/[code]`, `label-it` at `/label-it/[code]`, `true-false` at `/true-false/[code]`, `type-answer` at `/type-answer/[code]`, `spelling-practice` at `/spelling/[code]`, `fill-in-the-blank` at `/fill/[code]`, and `balloon-pop` at `/balloon/[code]`.
