...
- Confirm `/teacher/review/sample-publisher` shows `Sample Publisher Lab tenant draft review queue`, `Tenant scope: only Sample Publisher Lab draft items appear on this route.`, and `AI-generated daily routines draft preview` without MiniStar draft content.
- Confirm `/teacher/review/ministar` shows `MiniStar English Lab tenant draft review queue`, `Tenant scope: only MiniStar English Lab draft items appear on this route.`, and `AI-generated MiniStar greetings draft preview` without sample-publisher draft content.
...
- Confirm `/teacher/review` keeps AI-generated draft review blocked with `AI package writer gate pending`, `MiniStar AI package writer gate pending`, `Package writer harness implementation decision required`, and `Package writer harness decision evidence` before package writing, route creation, playlist creation, local bundles, or assignment shells can exist.
