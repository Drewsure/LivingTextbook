...
- MiniStar has tenant-specific prompt package, cost gate, request builder, audio coverage, gamification mapping, reward readiness, engine binding, Draft JSON preview, and derived correction queue seed records for the Level 1 greetings generator request.
- MiniStar verifier submission, generated package manifest, publish readiness, audio approval, media-rights, and teacher approval lanes remain blocked until those request-specific records are created and reviewed.
