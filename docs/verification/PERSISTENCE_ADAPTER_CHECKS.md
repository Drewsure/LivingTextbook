...
8. Confirm hosted write intents include route registry, teacher draft packages, teacher draft review handoff packets, teacher draft reviewer decisions, teacher draft review evidence packets, teacher launch-session settings, progress/media events, package game/audio coverage snapshots, package release candidate status, package publish gates, and package approval ledgers.
9. Confirm local write intents include local media bundle manifests, local teacher draft packages, local teacher draft review handoff packets, local teacher draft reviewer decisions, local teacher draft review evidence packets, local package game/audio coverage snapshots, local progress export packages, local package release candidate status, local package publish gates, and local package approval ledgers.
...
17. Confirm package approval ledger write intents require policy before real signatures are stored.
18. Confirm package release candidate and package publish gate write intents are required before pilot release.
19. Confirm core write intents reject raw learner audio.
20. Confirm core write intents reject learner transcripts.
21. Confirm export-capable intents are policy gated.
22. Confirm local-classroom intents show offline support.
23. Confirm local-classroom handoff steps mention backup/restore and release-control export or restore.
24. Confirm hosted handoff steps mention migration/table mapping and release-gate validation before publish status changes.
25. Confirm handoff steps are visible for each adapter plan.
26. Confirm the shared contract exists at `packages/content-model/src/persistenceAdapter.ts`.
27. Confirm the sample data exists at `apps/web/src/data/samplePersistenceAdapterPlan.ts`.
28. Confirm the UI panel exists at `apps/web/src/features/persistence/PersistenceAdapterReadinessPanel.tsx`.
29. Confirm typecheck passes after pulling latest.
30. Confirm production build passes after pulling latest.
