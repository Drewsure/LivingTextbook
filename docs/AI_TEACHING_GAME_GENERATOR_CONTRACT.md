...
Teacher generator routes may show an AI generated package teacher review packet after verifier result evidence planning and before generated package manifests. The packet is a teacher approval prep surface: it gathers generated content fit, target-language audio, curated activity pathway, deterministic rewards, media-rights evidence, support-language boundaries, verifier result evidence, missing evidence, blocked actions, and next required records into one readable review packet.

AI generated package teacher review packets must pass the shared `validateAiGeneratedPackageTeacherReviewPacket` guard before teacher approval capture, package assembly, route creation, playlist creation, assignment creation, local bundle writes, student-ready markers, or support-language progress can be considered. The guard requires decision-lane topics, teacher questions, missing evidence, next records, `ai_verifier_result_evidence_packet`, target-language audio approval, media-rights evidence, release-control binding, assignment rollout, and blocked package actions.

The packet cannot capture teacher approval from verifier result evidence, assemble packages, write route registry entries, create playlists, create assignments, write local bundles, mark packages student-ready, or treat support-language activity as progress evidence. It must keep `ai_verifier_result_evidence_packet`, `teacher_approval_ledger`, `media_rights_evidence_attachment`, `target_language_audio_approval`, `release_control_binding`, and `assignment_rollout_gate` visible as future records.
