...
The MiniStar Level 1 greetings generator seed may include tenant-specific prompt package, cost gate, disabled request builder, audio coverage, gamification mapping, reward readiness, and engine binding records. That seed does not make MiniStar generator-ready until Draft JSON preview, verifier submission, generated package manifest, publish readiness, and correction queue records also exist and pass review.
