...
export const sampleWhiteLabelReleaseReadiness: WhiteLabelReleaseReadiness = {
  ...
  browserEvidenceMode: "coded-rehearsal",
  status: "blocked",
  phases: [
    {
  ...
  qualityEvidence: ([
    ["typecheck", "Web typecheck", "command", "typecheck:web", ["apps/web", "shared content model"]],
    ["productionBuild", "Production build", "command", "web-production-build", ["apps/web", "webpack production output"]],
    ["activeRoutes", "Active route sweep", "route-sweep", "active-route-verification", ["89 active routes", "teacher and student pathways"]],
    ["runtime", "Runtime composition", "command", "verify:foundation-composition", ["shared runtime contracts", "no-side-effect guards"]],
    ["browser", "Browser rehearsal", "browser-rehearsal", "browser-rehearsal-evidence", ["/launch/demo-unit-1", "/teacher/sessions/demo-unit-1", "coded local evidence"]],
    ["privacy", "Privacy boundary", "privacy-negative-test", "privacy-boundary-verification", ["raw audio exclusion", "transcript exclusion", "support-language progress exclusion"]],
    ["tenantIsolation", "Tenant isolation", "tenant-negative-test", "tenant-isolation-verification", ["cross-tenant read rejection", "cross-tenant write rejection", "tenant-scoped evidence"]],
  ] as const).map(([checkId, label, evidenceKind, sourceRecord, scope]) => ({
    ...
    label,
    verified: true,
    evidenceKind: evidenceKind as WhiteLabelReleaseReadiness["qualityEvidence"][number]["evidenceKind"],
    sourceRecord,
    scope: [...scope],
