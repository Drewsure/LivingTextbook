...
export const sampleBackendSchemaDraft: BackendSchemaDraft = {
  ...
  entities: [
    ...
    },
    {
      ...
      deploymentFit: "hybrid",
      purpose: "Stores teacher-created classroom sessions, entry codes, settings snapshot, microphone approval, support-language visibility, and report controls.",
      fields: [
      ...
      indexes: ["session_id unique", "tenant_id + teacher_code", "tenant_id + package_id", "settings_revision"],
      forbiddenFields: ["Raw learner audio", "Learner transcript", "Ungated AI Tutor setting", "Support-language mastery override"],
      migrationNote: "Policy and role access are required before live classroom storage. The settings snapshot must preserve non-scoring support-language and background-media flags.",
    },
    {
      ...
      indexes: ["session_id + created_at", "session_id + learner_code", "session_id + event_type", "session_id + event_effect"],
  ...
  crossCuttingRules: [
    ...
    "Launch sessions must preserve teacher settings snapshots so support-language, microphone, background-media, AI Tutor, Training Academy, and reporting controls are auditable.",
