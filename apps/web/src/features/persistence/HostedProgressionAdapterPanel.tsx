...
export function HostedProgressionAdapterPanel({ request }: HostedProgressionAdapterPanelProps) {
  ...
  const tone = result?.status === "available" ? "success" : result?.status === "error" ? "warning" : "neutral";
  const label = checking ? "Checking" : result?.status === "available" ? "Available" : result?.status === "not-found" ? "No record" : result?.status === "error" ? "Unavailable" : "Not checked";
