...
import type { ShareEmbedReadinessPlan, ShareEmbedStatus } from "@/data/sampleShareEmbedReadinessPlan";

interface ShareEmbedReadinessPanelProps {
  plan: ShareEmbedReadinessPlan;
}

const statusTone: Record<ShareEmbedStatus, "neutral" | "success" | "warning"> = {
  blocked: "warning",
  optional: "neutral",
  planned: "neutral",
  ready: "success",
...
export function ShareEmbedReadinessPanel({ plan }: ShareEmbedReadinessPanelProps) {
  const publicBlockers = plan.gates.filter((gate) => gate.blocksPublicRelease && gate.status !== "ready").length;
  const blockedLanes = plan.lanes.filter((lane) => lane.status === "blocked").length;

  return (
    <Card>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-sm font-semibold text-[var(--tenant-muted)]">Share and embed readiness</p>
        ...
        <StatusPill label={publicBlockers > 0 ? "Public sharing blocked" : "Private share ready"} tone={publicBlockers > 0 ? "warning" : "success"} />
      </div>
      ...
      <dl className="mt-5 grid gap-3 sm:grid-cols-3">
        <ShareEmbedMetric label="Share lanes" value={String(plan.lanes.length)} />
        <ShareEmbedMetric label="Blocked lanes" value={String(blockedLanes)} />
        <ShareEmbedMetric label="Public blockers" value={String(publicBlockers)} />
      </dl>
        ...
        {plan.lanes.map((lane) => (
          <section key={lane.laneId} className="rounded-lg border border-[var(--tenant-border)] p-4">
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              <ShareEmbedList title="Allowed use" items={lane.allowedUse} />
              <ShareEmbedList title="Blocked use" items={lane.blockedUse} />
            </div>
        ...
        {plan.gates.map((gate) => (
          <section key={gate.gateId} className="rounded-lg border border-[var(--tenant-border)] bg-[var(--tenant-primary-soft)] p-4">
            ...
            <p className="mt-2 text-xs font-semibold uppercase text-[var(--tenant-muted)]">
              {gate.blocksPublicRelease ? "Blocks public release" : "Does not block public release"}
            </p>
...
function ShareEmbedMetric({ label, value }: { label: string; value: string }) {
  return (
...
function ShareEmbedList({ title, items }: { title: string; items: string[] }) {
  return (
